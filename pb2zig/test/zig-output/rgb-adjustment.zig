// Pixel Bender kernel "adjust" (translated using pb2zig)
const std = @import("std");

pub const kernel = struct {
    // kernel information
    pub const namespace = "example";
    pub const vendor = "example";
    pub const version = 1;
    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                const multipliers = self.params.multipliers;
                const src = self.input.src;
                const dst = self.output.dst;
                self.dst = @splat(0.0);

                self.dst = src.sampleNearest(self.outCoord());
                self.dst = @shuffle(f32, self.dst, @shuffle(f32, self.dst, undefined, @Vector(3, i32){ 0, 1, 2 }) * multipliers, @Vector(4, i32){ -1, -2, -3, 3 });
