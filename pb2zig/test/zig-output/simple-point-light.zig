// Pixel Bender kernel "SimplePointLight" (translated using pb2zig)
    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                ...
                const attenuationDelta = self.params.attenuationDelta;
                const attenuationSpeed = self.params.attenuationSpeed;
                const attenuationDecay = self.params.attenuationDecay;
                const src = self.input.src;
                const dst = self.output.dst;
                self.dst = @splat(0.0);

                const out_pixel_coord: @Vector(2, f32) = self.outCoord();
                const attenuation: f32 = pow(attenuationDelta / pow(distance(out_pixel_coord, center), attenuationDecay), attenuationSpeed);
