// Pixel Bender kernel "HighContrastFilter" (translated using pb2zig)
    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                const threshhold = self.params.threshhold;
                const invert = self.params.invert;
                const src = self.input.src;
                const dst = self.output.dst;
                self.dst = @splat(0.0);

                const px: @Vector(4, f32) = src.sampleNearest(self.outCoord());
                const brightest: f32 = max(px[0], max(px[1], px[2]));
                var v: f32 = undefined;
                if (invert > 0) {
