// Pixel Bender kernel "FocusingLinearBlur" (translated using pb2zig)
    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                const lineEquation = self.params.lineEquation;
                const uScale = self.params.uScale;
                const vScale = self.params.vScale;
                ...
                val *= 0.01;
                const dir: @Vector(2, f32) = @as(@Vector(2, f32), @splat(val)) * @shuffle(f32, lineEquation, undefined, @Vector(2, i32){ 0, 1 });
