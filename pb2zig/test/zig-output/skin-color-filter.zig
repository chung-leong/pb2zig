    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                const CbMin = self.params.CbMin;
                const CbMax = self.params.CbMax;
                const CrMin = self.params.CrMin;
                const CrMax = self.params.CrMax;
                const src = self.input.src;
                const dst = self.output.dst;
                self.dst = @splat(0.0);

                self.dst = src.sampleNearest(self.outCoord());
                const ycbcr: @Vector(3, f32) = @"V * M"(@shuffle(f32, self.dst, undefined, @Vector(3, i32){ 0, 1, 2 }), [3]@Vector(3, f32){
                    .{ 0.0, 0.0, 0.0 },
                    .{
                        -0.168736,
                        -0.331264,
