    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                ...
                var ycbcr: @Vector(3, f32) = @"V * M"(@shuffle(f32, self.dst, undefined, @Vector(3, i32){ 0, 1, 2 }), [3]@Vector(3, f32){
                    .{ 0.0, 0.0, 0.0 },
                    .{
                        -0.168736,
                        -0.331264,
                ...
                if ((ycbcr[1] >= CbMin) and (ycbcr[1] <= CbMax) and (ycbcr[2] >= CrMin) and (ycbcr[2] <= CrMax)) {
                    self.dst = @shuffle(f32, self.dst, @as(@Vector(3, f32), @splat(1)), @Vector(4, i32){ -1, -2, -3, 3 });
                } else {
                    self.dst = @shuffle(f32, self.dst, @as(@Vector(3, f32), @splat(0)), @Vector(4, i32){ -1, -2, -3, 3 });
                }
