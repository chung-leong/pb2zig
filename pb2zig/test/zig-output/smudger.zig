// Pixel Bender kernel "smudge" (translated using pb2zig)
    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                ...
                const nx: f32 = 5.0 * cos(pos[1] / 5.0);
                const ny: f32 = 5.0 * cos(pos[0] / 6.0);
                const pnt: @Vector(2, f32) = .{ nx * ny, nx * ny };
                self.dst = src.sampleNearest(self.outCoord() + pnt * @as(@Vector(2, f32), @splat(amount)));
