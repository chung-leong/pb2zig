// Pixel Bender kernel "BilateralBlur" (translated using pb2zig)
...
pub const kernel = struct {
    ...
    pub const version = 1;
    pub const description = "Bilateral Blur.";
    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                ...
                const r = self.params.r;
                const src = self.input.src;
                const dst = self.output.dst;
                self.dst = @splat(0.0);

                var pt: @Vector(2, f32) = undefined;
                var curPt: @Vector(2, f32) = undefined;
                var p0: @Vector(4, f32) = undefined;
                var p: @Vector(4, f32) = undefined;
                var tempP: @Vector(4, f32) = undefined;
                var totalP: @Vector(4, f32) = undefined;
                var w: f32 = undefined;
                var totalWeight: f32 = undefined;
                curPt = self.outCoord();
                totalP = @Vector(4, f32){ 0.0, 0.0, 0.0, 0.0 };
                totalWeight = 0.0;
                p0 = src.sampleNearest(curPt);
                pt[0] = curPt[0] - 4.0;
                pt[1] = curPt[1] - 4.0;
                p = src.sampleNearest(pt);
                tempP = abs(p0 - p);
                w = exp(-0.5 * (pow(5.656854249492381 / d, 2.0) + pow((tempP[0] + tempP[1] + tempP[2] + tempP[3]) / r, 2.0)));
                p *= @as(@Vector(4, f32), @splat(w));
                totalP += p;
                totalWeight += w;
                pt[1] += 1.0;
                p = src.sampleNearest(pt);
                ...
                totalP += p;
                totalWeight += w;
                self.dst = totalP / @as(@Vector(4, f32), @splat(totalWeight));
