// Pixel Bender kernel "DifferenceKey" (translated using pb2zig)
...
pub const kernel = struct {
    ...
    pub const version = 1;
    pub const description = "A simple difference key for chroma keying";
    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                const keyColor = self.params.keyColor;
                const tolerance = self.params.tolerance;
                const ramp = self.params.ramp;
                ...
                const diff: f32 = length(@shuffle(f32, self.dst, undefined, @Vector(3, i32){ 0, 1, 2 }) - keyColor);
                if (diff < tolerance) {
                    self.dst[3] = 0.0;
                } else if (diff < tolerance + ramp) {
                    self.dst[3] = pow((diff - tolerance) / ramp, gamma);
                }
