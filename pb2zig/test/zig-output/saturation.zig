    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                ...
                const satVal: f32 = saturation(cs);
                if (color[0] <= color[1]) {
                    if (color[1] <= color[2]) {
                        color[1] -= color[0];
                        color[2] -= color[0];
                ...
                const adjVec: @Vector(3, f32) = @shuffle(f32, cb, undefined, @Vector(3, i32){ 0, 1, 2 }) - @shuffle(f32, color, undefined, @Vector(3, i32){ 0, 1, 2 });
                const adjustment: f32 = luminance(adjVec);
                const adjustedColor: @Vector(3, f32) = color + @as(@Vector(3, f32), @splat(adjustment));
