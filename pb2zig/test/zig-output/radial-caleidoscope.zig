// Pixel Bender kernel "RadialCaleidoscope" (translated using pb2zig)
    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                const angle = self.params.angle;
                const direction = self.params.direction;
                const basepoint = self.params.basepoint;
                const src = self.input.src;
                const dst = self.output.dst;
                self.dst = @splat(0.0);

                const po: @Vector(2, f32) = self.outCoord() - basepoint;
                const theta: f32 = atan2(po[1], po[0]) - direction + 2.0 * DOUPLEPI;
                const radius: f32 = sqrt(po[0] * po[0] + po[1] * po[1]);
                var newAngle: f32 = mod(theta, angle);
                const section: f32 = floor(theta / angle);
                if (mod(section, 2.0) > 0.5) {
                    newAngle = angle - newAngle;
                }
                newAngle += direction;
                const tmp1 = newAngle;
                self.dst = src.sampleLinear(basepoint + @Vector(2, f32){
                    radius * cos(tmp1),
                    radius * sin(tmp1),
                });
