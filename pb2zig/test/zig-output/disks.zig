    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type, comptime ParameterStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                ...
                const rotationR: [2]@Vector(2, f32) = inverse2x2(rotation);
                const rot: [2]@Vector(2, f32) = @"M / S"(@"M * M"(rotation, hcs), size);
                const rotR: [2]@Vector(2, f32) = @"M * M"(@"M * S"(hcsR, size), rotationR);
                const p: @Vector(2, f32) = floor(@"V * M"((self.outCoord() - base), rot) + @as(@Vector(2, f32), @splat(0.5)));
                ...
                p1 = @"V * M"((p + @Vector(2, f32){ -1.0, -1.0 }), rotR) + base - self.outCoord();
                pxl = src.sampleNearest(p1 + self.outCoord());
                dist = self.distanceSquared(p1);
                self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                p1 = @"V * M"((p + @Vector(2, f32){ -1.0, 0.0 }), rotR) + base - self.outCoord();
                pxl = src.sampleNearest(p1 + self.outCoord());
                dist = self.distanceSquared(p1);
                self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                p1 = @"V * M"((p + @Vector(2, f32){ 0.0, -1.0 }), rotR) + base - self.outCoord();
                pxl = src.sampleNearest(p1 + self.outCoord());
                dist = self.distanceSquared(p1);
                self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                p1 = @"V * M"((p + @Vector(2, f32){ 0.0, 1.0 }), rotR) + base - self.outCoord();
                ...
                self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                if (radius > 2.25) {
                    p1 = @"V * M"((p + @Vector(2, f32){ -2.0, 0.0 }), rotR) + base - self.outCoord();
                    pxl = src.sampleNearest(p1 + self.outCoord());
                    dist = self.distanceSquared(p1);
                    self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                    p1 = @"V * M"((p + @Vector(2, f32){ -2.0, -1.0 }), rotR) + base - self.outCoord();
                    pxl = src.sampleNearest(p1 + self.outCoord());
                    dist = self.distanceSquared(p1);
                    self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                    p1 = @"V * M"((p + @Vector(2, f32){ -1.0, -2.0 }), rotR) + base - self.outCoord();
                    pxl = src.sampleNearest(p1 + self.outCoord());
                    dist = self.distanceSquared(p1);
                    self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                    p1 = @"V * M"((p + @Vector(2, f32){ 0.0, -2.0 }), rotR) + base - self.outCoord();
                    pxl = src.sampleNearest(p1 + self.outCoord());
                    dist = self.distanceSquared(p1);
                    self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                    p1 = @"V * M"((p + @Vector(2, f32){ 0.0, 2.0 }), rotR) + base - self.outCoord();
                    ...
                    self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                    if (radius > 3.0) {
                        p1 = @"V * M"((p + @Vector(2, f32){ -2.0, -2.0 }), rotR) + base - self.outCoord();
                        pxl = src.sampleNearest(p1 + self.outCoord());
                        dist = self.distanceSquared(p1);
                        self.pxlOut += @as(@Vector(4, f32), @splat(clamp(0.75 * size * (1.0 - dist / radius), 0.0, 1.0))) * pxl;
                        p1 = @"V * M"((p + @Vector(2, f32){ 2.0, 2.0 }), rotR) + base - self.outCoord();
                ...
                pxlOut.writePixel(self.outputCoord, self.pxlOut);
            }
...
pub fn KernelInput(comptime Kernel: type) type {
    const info = @typeInfo(@TypeOf(Kernel.inputImages)).@"struct";
    const len = info.field_names.len;
...
pub fn KernelInputImpl(comptime Impl: type, comptime Kernel: type) type {
    ...
    var field_attrs: [len]std.builtin.Type.Struct.FieldAttributes = undefined;
    inline for (info.field_names, 0..) |field_name, index| {
        const input = @field(Kernel.inputImages, field_name);
        field_names[index] = field_name;
...
pub fn KernelOutput(comptime Kernel: type) type {
    const info = @typeInfo(@TypeOf(Kernel.outputImages)).@"struct";
    const len = info.field_names.len;
    var field_names: [len][]const u8 = undefined;
    var field_types: [len]type = undefined;
    var field_attrs: [len]std.builtin.Type.Struct.FieldAttributes = undefined;
...
pub fn KernelOutputImpl(comptime Impl: type, comptime Kernel: type) type {
    ...
    var field_attrs: [len]std.builtin.Type.Struct.FieldAttributes = undefined;
    inline for (info.field_names, 0..) |field_name, index| {
        const output = @field(Kernel.outputImages, field_name);
        field_names[index] = field_name;
...
pub fn KernelParameters(comptime Kernel: type) type {
    const info = @typeInfo(@TypeOf(Kernel.parameters)).@"struct";
    const len = info.field_names.len;
    var field_names: [len][]const u8 = undefined;
    var field_types: [len]type = undefined;
    var field_attrs: [len]std.builtin.Type.Struct.FieldAttributes = undefined;
    inline for (info.field_names, 0..) |field_name, index| {
        const param = @field(Kernel.parameters, field_name);
