    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                ...
                const focalSize = self.input.focalSize;
                const invert = self.input.invert;
                const amount = self.input.amount;
                const edgeHardness = self.input.edgeHardness;
                const vignette = self.input.vignette;
                
                var coord: @Vector(2, f32) = self.outCoord();
                var cur_radius: f32 = length(coord - center);
                var color: @Vector(4, f32) = self.input.src.sampleNearest(coord);
