    ...
    fn Instance(comptime InputStruct: type, comptime OutputStruct: type) type {
        return struct {
            ...
            pub fn evaluatePixel(self: *@This()) void {
                ...
                offset[0] = 0.0;
                offset[1] = 0.0;
                p0 = self.input.src.sampleLinear(p + offset);
                offset[0] = -dist;
                offset[1] = -dist;
                p1 = self.input.src.sampleLinear(p + offset);
                offset[0] = 0.0;
                offset[1] = -dist;
                p2 = self.input.src.sampleLinear(p + offset);
                offset[0] = dist;
                offset[1] = -dist;
                p3 = self.input.src.sampleLinear(p + offset);
                offset[0] = dist;
                offset[1] = 0.0;
                p4 = self.input.src.sampleLinear(p + offset);
                offset[0] = dist;
                offset[1] = dist;
                p5 = self.input.src.sampleLinear(p + offset);
                offset[0] = 0.0;
                offset[1] = dist;
                p6 = self.input.src.sampleLinear(p + offset);
                offset[0] = -dist;
                offset[1] = dist;
                p7 = self.input.src.sampleLinear(p + offset);
                offset[0] = -dist;
                offset[1] = 0.0;
                p8 = self.input.src.sampleLinear(p + offset);
                c = w;
